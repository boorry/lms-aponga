...
- les rôles de référence (`learner`, `teacher`, `content_author`, `academy_manager`, `administrator`, `guardian`).

Le seed doit être idempotent (rejouable sans dupliquer le compte Administrator).

## Acceptance
Migration from scratch + seed + tests invariants passent. Connexion possible avec le compte Administrator seedé.
